...
class rozetka {
	...
	// Проверяет наличие файла, вызывает recFunction, выводит результаты из array_group
    ...
    public function fgphv_virtuemart_categories($ids)
	{
        ...
        $q = mysql_query("UPDATE `" . $this->jprefix . "_virtuemart_categories` SET `id_1c` = '" 
    ...
    public function parseXmlFile($FileName, $FileNamePrice)
	{
       ...
       (string) $item->Наименование[0] = str_replace("  ", " ", (string) $item->Наименование[0]);
        ...
        $array = mysql_fetch_array($q);
        if ($array["id_1c"]=="")  {
            $this->add_fgphv_virtuemart_products($array_data["name"],$array_data["price"], $item->Группы->Ид, $item->Ид[0], $item->Артикул[0]);
    ...
    // Добавляем в fgphv_virtuemart_product_prices
    public function add_fgphv_virtuemart_products($name, $price, $idg, $id, $art){
    ...
    public function importImagesProducts($size, $quality, $export){
